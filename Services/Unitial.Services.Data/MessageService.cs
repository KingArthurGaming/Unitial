...
namespace Unitial.Services.Data
{
    public class MessageService : IMessageService
    {
        ...
        public IQueryable<Message> GetNewMessages(string lastMessage, string receiverId, string uesrId)
        {
            var date = DateTime.Parse(lastMessage);
            var conversationId = conversationService.GetConversationId(receiverId, uesrId);
            var newMessages = messageRepository.All().Where(x => x.ConversationId == conversationId && x.SenderId == receiverId && x.SendedOn > date);
            return newMessages;
        }
